...
public class SettlementCompleteAction extends ActionSupport implements SessionAware{
	...
	public String execute() {
		...
		int count = 0;
		for(int i=0; i<purchaseHistoryInfoDtoList.size();i++) {		 //購入履歴リストの要素数だけ処理を繰り返す
			count += purchaseHistoryInfoDAO.regist(
					String.valueOf(session.get("loginId")), //sessionからloginIdを取得しString型へ
		...
		if(count > 0) {
			...
			if(count > 0) {
				...
				if(!(iterator.hasNext())) {//もし次のデータがないときcartInfoListにnullを代入
					cartInfoDtoList = null;
				}
				session.put("cartInfoDtoList", cartInfoDtoList);//セッションにcartInfoDtoListを入れる
